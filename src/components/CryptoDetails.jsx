...
const CryptoDetails = () => {
    ...
    const time = ['3h', '24h', '7d', '30d', '1y', '3m', '3y', '5y'];
     ...
     const stats = [
    { title: 'Price to USD', value: `$ ${millify(data?.data?.coin.price) }`, icon: <DollarCircleOutlined /> },
    { title: 'Rank', value: data?.data?.coin.rank, icon: <NumberOutlined /> },
    { title: '24h Volume', value: `$ ${millify(data?.data?.coin.volume)}`, icon: <ThunderboltOutlined /> },
    { title: 'Market Cap', value: `$ ${millify(data?.data?.coin.marketCap)}`, icon: <DollarCircleOutlined /> },
    { title: 'All-time-high(daily avg.)', value: `$ ${ millify(data?.data?.coin.allTimeHigh.price)}`, icon: <TrophyOutlined /> },
  ];
   console.log(stats);

  const genericStats = [
    { title: 'Number Of Markets', value: `${millify(data?.data?.coin.numberOfMarkets)}`, icon: <FundOutlined /> },
